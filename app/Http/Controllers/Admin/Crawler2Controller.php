...
class Crawler2Controller extends Controller
{
    ...
    private function insertChap($value)
    {
        ...
        if(!empty($chapTitles) && !empty($chapUrls)) {
            foreach($chapUrls as $k => $v) {
                ...
                $slug = $epchapArray[4];
                // check post_eps
                $postEp = PostEp::where('slug', $slug)->where('post_id', $value->id)->first();
                if(isset($postEp)) {
                    break;
                }
                // name
    ...
    public function truyenfullpostchapep(Request $request) {
        Cache::flush();
        trimRequest($request);
